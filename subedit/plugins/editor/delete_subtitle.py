...
from subedit.database.database import deleteSubtitleDocument, getSubtitleID, getCollabAdmin
...
async def deleteSubtitle(_, query):
    ...
    collab_admin = getCollabAdmin(sub_id)
    if collab_admin != user_id:
        await query.answer("⛔️ You are not authorized to delete this subtitle.", show_alert=True)
        return
    else:
